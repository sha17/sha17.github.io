...
1. **Meta Representation: What?**<br>
  어떤 Meta Knowledge를 학습하는가?<br>
  Optimizer, Initialization, Metric 등의 예가 있다.
2. **Meta optimizer: How?**<br>
  Meta Knowledge를 어떤 방법으로 학습하는가?<br>
  Gradient Descent, Reinforcement Learning, Evolutionary Search 등이 있다.
3. **Meta Objective: Why?**<br>
  Meta Knowledge를 사용해 무엇을 하는가?<br>
  $\mathcal{L^{meta}}$, Task Distribution, 그리고 inner/outer loop간의 데이터 흐름 방식에 따라 결정된다.<br>
  Few shot learning, adversarial attack, MAML 등이 있겠다.

## 내 생각
Meta Learning의 개념이나 전체적인 개요가 굉장히 애매모호해서 그전의 논문들이나 블로그 글 등을 읽어봐도 한번에 이해하기가 어려웠는데(내 이해력이 모자랐을수도), 이 논문은 Meta Learning에 대한 개념과 구성요소에 대해 정말 깔끔하게 정리하여 궁금했던 것들이 모두 해결되었다. 특히 다른 연관 분야와 구분점과 비슷한 점에 대하여 기술해 놓은 것이 후에 관련 필드를 찾을때 유익할 것 같다. 앞으로 읽는 Meta Learning Paper들은 이 논문에서 제시한 Category들을 사용하여 정리할 것이다.
